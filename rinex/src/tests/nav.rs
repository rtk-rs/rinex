use crate::{
    navigation::{NavFrameType, NavMessageSubtype, NavMessageType},
...
fn nav_v4_02_spec_examples() {
    // RINEX 4.02 specification examples (Tables A12, A18, A32, A41)
    // gathered in a single 4.02 file
    let path = env!("CARGO_MANIFEST_DIR").to_owned()
        + "/../test_resources/NAV/V4/rinex402_examples_MN.rnx";
    ...
    // share a key: the file has 35 records for 30 keys.
    ...
    assert_eq!(sto.as_system_time().unwrap().t_tm, 604080);
    let (_, sto) = find(NavFrameType::SystemTimeOffset, "R26", NavMessageType::LXOC);
    assert_eq!(sto.as_system_time().unwrap().t_tm, 518400);
